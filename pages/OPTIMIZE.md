...
- All rooms index: https://dennisyu.com/ai-implementation-workshops/ (old `/workshops/` redirects)
- Attendee homework: https://dennisyu.com/workshop-prep/
...
Same chrome on `/dealcon` `/wichita` `/digimarcon` `/jva` `/ai-implementation-workshops` `/affiliateworld` `/activate` `/dunk-playbook` `/dunkademics` `/workshop`. Dunk Camp replica `dunkerspotlight.com/set-up-claude` is a different WordPress — no creds in this vault.

**Index of replica rooms:** https://dennisyu.com/ai-implementation-workshops/ (not `/speaking/`, which is the booking page). Attendee homework for every room: https://dennisyu.com/workshop-prep/
